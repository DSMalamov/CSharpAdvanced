Queue<int> coffeeInput = new(Console.ReadLine().Split(", ", StringSplitOptions.RemoveEmptyEntries).Select(int.Parse));
Stack<int> milkInput = new(Console.ReadLine().Split(", ", StringSplitOptions.RemoveEmptyEntries).Select(int.Parse));

Dictionary<string, int> table = new Dictionary<string, int>();
Dictionary<string, int> drinksMade = new();
...
{
    Console.WriteLine(string.Join(", ", coffeeInput));
}
else
{
...
{
    Console.WriteLine(string.Join(", ", milkInput));
}
else
{
...
drinksMade.OrderBy(x => x.Value).ThenBy(y => y.Key);
